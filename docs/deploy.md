...
**系統怎麼判斷哪一列是標題列**：解析時會自動找第一個「有商品名稱欄、且對到至少 3 個範本欄位」的列當標題列，這一列之前的列（蝦皮匯出檔開頭常見的說明文字列）會被跳過，不當成資料處理。標題列之後，**同一個商品的第二條規格以後，那一列可以不填商品編號、也不填商品名稱**，系統會自動把它接到上一個商品當成同一個商品的另一個規格；但如果一列填了商品名稱、卻沒填商品編號，會被當成錯誤擋下來，訊息是「這一列有商品名稱但沒有商品編號」。手動編輯範本時要記得：**每個新商品的第一列一定要填商品編號**，之後同一個商品的其他規格列才能省略。

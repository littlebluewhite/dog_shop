...
**價格、庫存、SKU 的規則**：「價格」是必填欄位，留白會報錯「價格必填」；上限是 9,999,999，超過會報錯「價格最多 9,999,999」。「庫存」可以留白，新商品留白庫存算 0，更新既有商品時留白會維持原本的庫存不變。「SKU」也可以留白，最多 60 字。

**分類是照名稱找或建立的**：工作表「分類」欄打的名稱如果跟後台現有的分類一字不差，就會用那個既有分類；只要有一個字不一樣（例如多打一個空格、簡繁體不同），系統不會報錯，會直接建立一個新的同義分類，新分類的網址代稱是隨機產生的一串英數字。所以「分類」欄要跟後台分類頁的名稱**完全一致**；匯入後也建議去分類頁看一下有沒有意外多出來的分類，需要的話手動合併或刪除。

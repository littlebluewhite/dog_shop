...
只有一種情況要改：如果之後在 Caddy **前面**又加一層 CDN（例如 Cloudflare），就要讓 Caddy 知道該信任誰，不然所有請求在 api 眼中來源 IP 都會變成 CDN 自己的 IP，限流形同虛設。做法是在 `deploy/Caddyfile` 最上面加一個全域設定區塊：
...
- **還原半路失敗**：如果 `./restore.sh <檔名>` 跑到一半失敗（例如檔名打錯、備份檔不存在或壞掉），腳本會**自己把 `api`／`web` 重新啟動**再結束，店不會停在關閉狀態；畫面上不會出現最後那一行「完成」，照錯誤訊息去查原因就好。萬一連自動啟動都沒成功（`docker compose ps` 看到 `api`／`web` 沒在跑），手動執行 `docker compose start api web` 讓服務恢復。另外，檔名只收 `dog_shop-….dump` 這種形狀，打錯形狀會直接被擋下來、**不會先停掉 `api`／`web`**。
...
**圖片下載時間**：伺服器會自己去下載「圖片網址」欄裡的網址，每張圖最多等 10 秒、最多 6 張同時下載、單張上限 10 MB。用 500 個商品、每個商品 9 張圖估算，最壞情況大約要 2 小時，商品數量多時**建議分批匯入**（例如一次 100 個），不要一次全部上傳。這個下載過程最多用到 6 個並行連線，在建議的 2 vCPU／4 GB VPS 上沒問題；如果之後換成規格更小的主機，不建議在上面跑大批次匯入。**記憶體的最壞情況**：單張圖片解碼最多會用到約 256 MB 記憶體、最多 6 張同時，所以**不要在 4 GB 以下的機器跑大批匯入**。

**匯入期間的庫存覆寫窗口**：匯入是**先**把現有商品的資料（含當下的庫存）讀一次，**接著**才花時間下載圖片，**最後**把結果寫回資料庫；寫回時用的是一開始讀到的那份舊資料。所以在這段時間裡，庫存的三個變動來源都會被蓋掉——**你自己編輯商品**、**處理訂單出貨**，還有**客人下單**（客人下單會扣庫存，這個你停不掉）。範圍講精確一點：**只影響「庫存」欄留白的規格**；「庫存」欄有填數字的，一律以工作表為準，本來就是你要的結果，不存在被蓋掉的問題。

要避開這個窗口，兩個方法挑一個：**要嘛把「庫存」欄整欄填滿**（填了就以工作表為準，等於沒有這個窗口），**要嘛挑夜間離峰時間分批匯入**（一次 100 個商品，把每一批的時間壓短）。批次數量大、預期匯入要跑比較久時，這一點特別重要——依上一段的估算，500 個商品最壞要跑 2 小時，那就是 2 小時的窗口。

**新增的商品，狀態是「草稿」**，要自己到後台檢查沒問題後再上架。**更新既有商品時不會動到上架狀態**：工作表沒有「狀態」這一欄，已經上架中的商品被匯入更新（例如整批調價）之後還是上架中，不會被打回草稿。

重複匯入同一個「商品編號」，會拿現有商品當底更新：**網址代稱（slug）、上架狀態、排序這三項工作表沒有對應欄位，更新時一律維持原本的值**（上架狀態的細節見上一段）；**商品描述、分類、庫存**這幾項工作表有對應欄位，欄位留白時會保留原本的值，不會被清空或重置成預設值。**圖片網址留白、或填了但整批下載失敗，也會保留原圖；只要有一張下載成功，商品的圖片就整組換成這次下載到的（不是新舊圖片合併）**，所以圖片網址欄要嘛整組留白、要嘛整組填好填滿。
